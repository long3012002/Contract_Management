...
public class SystemCodeController : ControllerBase
{
    ...
    public async Task<IActionResult> GenerateDuAnCode([FromQuery] int loaiDuAn = 1, [FromQuery] int? nam = null)
    {
        var code = await _codeGeneratorService.GenerateDuAnCodeAsync(loaiDuAn, nam);
        return Ok(new { code });
    }

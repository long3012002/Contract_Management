...
const LoginFormCard = memo(function LoginFormCard({
  ...
  const { ref: usernameRegRef, ...usernameRegister } = register('username');
  const { ref: passwordRegRef, ...passwordRegister } = register('password');
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          {import.meta.env.DEV && (
            <Button
              ...
              disabled={isLoading}
              className="w-full mt-2 border-dashed border-red-300 hover:bg-red-50 text-red-600 hover:text-red-700"
            >
              🚀 Bypass Login (Admin)
            </Button>
          )}
